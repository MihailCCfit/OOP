...
public class SettingsView {
    ...
    public SettingsView(Screen screen, int speed) {
        ...
        initialisePositions();
        this.speed = speed;
        ConsoleUtils.printLine(screen, "Settings",
    ...
    private void initialisePositions() {
        TerminalSize size = screen.getTerminalSize();
        gameSpeedPosition = new TerminalPosition(Math.max(size.getColumns() / 2 - 7, 1),
    ...
    private void drawText() {
        ConsoleUtils.printLine(screen, "Settings",
                ...
                TextColor.ANSI.CYAN);
        String text = (canDecrease() ? "<" : " ") + "Speed |" + "#".repeat(speed) + " ".repeat(5 - speed)
                + "|" + (canIncrease() ? ">" : " ");
