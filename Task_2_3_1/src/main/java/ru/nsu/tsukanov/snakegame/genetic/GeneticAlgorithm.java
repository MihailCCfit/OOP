package ru.nsu.tsukanov.snakegame.genetic;
...
public class GeneticAlgorithm {
    ...
    @JsonIgnore
    private final Game gameForCopy;
    ...
    public List<Individual> generateGenes() {
        ...
        for (int curRound = 0; curRound < maxRounds; curRound++) {
            ...
            System.out.println(
                    "Best: " + Arrays.stream(individuals)
    ...
    private Individual[] selection(Individual[] individuals,
                                   Map<Integer, Integer> fitnessMap) {
    ...
    private void individualMutation(Individual individual) {
        ...
        for (int i = 0; i < genes.length; i++) {
            if (random.nextDouble() < chance) {
                double gen = genes[i];
                genes[i] = (random.nextDouble(gen - 0.05, gen + 0.05) +
                        random.nextDouble(gen - .05, gen + .05)) / 2;
            }
        }
    }


    private Map<Integer, Integer> averageFitnessOfPopulation(Individual[] individuals, Game game, int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            fitnessOfPopulation(individuals, game).forEach((id, score) -> {
                avgFitnessMap.put(id, avgFitnessMap.get(id) + score);
            });
        }
        avgFitnessMap.forEach((id, score) -> {
